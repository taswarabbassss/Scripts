    ...
    for (let skipValue = 0; skipValue <= totalDocumets; skipValue = skipValue + batchSize) {
        ...
        clientsList.forEach(client => {
            if (client.clientDocuments) {
                const clientDocumentsList = client.clientDocuments;
                const totalDocument = clientDocumentsList.length;
                let limit = 10;
                let endSize = limit;
                print("TOTAL: " + totalDocument)
                for (let skipDocs = 0; skipDocs <= totalDocument; skipDocs = skipDocs + limit) {
                    let clientDocumentsToIterate = clientDocumentsList.slice(skipDocs, endSize);
                    print(clientDocumentsToIterate.length);
                    endSize = endSize + limit;
                    clientDocumentsList.forEach(document => {
