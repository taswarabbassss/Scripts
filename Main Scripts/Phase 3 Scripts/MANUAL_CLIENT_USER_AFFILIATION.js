    ...
    for (
      ...
      sourceDocumentsList.forEach((sourceDocument) => {
        ...
        if (createrUser && modifierUser && clientObj) {
          ...
          sourceDocument.affiliatedUser.forEach((affiliatedUser) => {
...
const constructorParameters = {
  ...
  batchSize: 50,
  findQuery: {
    affiliatedUser: { $exists: true, $type: "array" },
    $expr: { $gte: [{ $size: "$affiliatedUser" }, 1] },
    // affiliatedUser: { $exists: true },
    // affiliatedUser: { $gte: [{ $size: "$affiliatedUser" }, 1] },
  },
};
