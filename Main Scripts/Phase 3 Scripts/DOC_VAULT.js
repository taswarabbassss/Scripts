  ...
  mainDataAssociationMethod() {
    ...
    for (
      ...
      sourceDocumentsList.forEach((sourceDocument) => {
        sourceDocument.clientDocuments.forEach((document) => {
          const createrUserId = this.getObjectId(document.uploadedBy);
          ...
          if (createrUser && modifierUser && clientObj) {
            ...
            const dateCreated = document?.uploadedAt;
            const dateModified = document?.uploadedAt;
            if (
...
const constructorParameters = {
  ...
  batchSize: 50,
  findQuery: {
    clientDocuments: { $exists: true, $type: "array" },
    $expr: { $gte: [{ $size: "$clientDocuments" }, 1] },
  },
};
...
/**
  ...
  createdAt,lastModifiedAt with uploadedAt
 */
