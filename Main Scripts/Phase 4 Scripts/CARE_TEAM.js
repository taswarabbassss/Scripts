class DataAssociation {
  ...
  finalLogs() {
    ...
    print(`${this.removeStatusDocuments}: Documents with RemoveStatus`);
    print(
  ...
  mainDataAssociationMethod() {
    ...
    for (
      ...
      let sourceDocumentsList = db
        ...
        .toArray();
      const sourceLength = sourceDocumentsList.length;
      sourceDocumentsList.forEach((sourceDocument) => {
        const clientObj = sourceDocument;
        if (clientObj) {
          const careTeamMembersStatusNotRemove = sourceDocument.careTeam.filter(
            (careTeamUser) => careTeamUser.status !== "REMOVE"
          );
          this.removeStatusDocuments =
            sourceLength - careTeamMembersStatusNotRemove.length;
          careTeamMembersStatusNotRemove.forEach((careTeamUser) => {
            const associatedUser = this.getUserWithId(careTeamUser._id);
            let createrUser = null;
            if (careTeamUser.addedByAnotherUser) {
              createrUser = this.getUserWithId(careTeamUser.addedByUserId);
            } else {
              createrUser = associatedUser;
            }
            if (associatedUser && createrUser) {
              const modifierUser = createrUser;
              this.setDefaultTenantId(createrUser, modifierUser);
              const dateCreated = careTeamUser?.joiningDate;
              const dateModified = careTeamUser?.joiningDate;
              if (
...
const constructorParameters = {
  ...
  batchSize: 50,
  findQuery: {
    careTeam: { $exists: true, $type: "array" },
    $expr: { $gte: [{ $size: "$careTeam" }, 1] },
  },
};
...
/**
  ...
  1) getDetailDocument
  2) addSummaryDocumentWhenDetailDocAlreadyExists (change the dateCreated and dateMofified)
  3) addNewDetailAndSummaryDocument (change the dateCreated and dateMofified)
 */
