class DataAssociation {
  ...
  getTotalCountAggregationPipeline() {
    return [
      ...this.findQuery,
      {
        $count: "totalDocs",
  ...
  getFindAggregationPipeLine(skip, limit) {
    return [
      ...this.findQuery,
      {
        $skip: skip,
  ...
  createMyIndexes() {
    const existingIndexes = this.db
      ...
      .getIndexes();
    const indexes = Object.keys(this.queryIndexes).filter(
  ...
  mainDataAssociationMethod() {
    ...
    for (
      ...
      const sourceDocumentsList = this.db
        .getCollection(this.sourceCollection)
        .aggregate(this.getFindAggregationPipeLine(skipValue, this.batchSize))
        // .skip(skipValue)
      ...
      sourceDocumentsList.forEach((sourceDocument) => {
        ...
        if (clientObj) {
          const allowedUsersStatus = ["REQUESTED", "ACCEPTED"];
          const filteredHistoryUsersList = sourceDocument.history.filter(
            (historyDoc) => allowedUsersStatus.includes(historyDoc.status)
          );
          filteredHistoryUsersList.forEach((historyDoc) => {
            const associatedUserId = historyDoc?.statusChangeBy?.userId;
            const associatedUser = this.getUserWithId(associatedUserId);
            if (associatedUser) {
              const createrUser = associatedUser;
              const modifierUser = associatedUser;
              this.setDefaultTenantId(createrUser, modifierUser);
              const dateCreated = historyDoc?.statusChangedOn;
              const dateModified = historyDoc?.statusChangedOn;
              let agency = null;
              if (historyDoc?.status === "REQUESTED") {
                agency = historyDoc?.recipientAgency;
              } else {
                agency = historyDoc?.statusChangeBy?.agency;
              }
  ...
  postCreationSetup() {
    ...
    this.totalDocuments = this.db
      .getCollection(this.sourceCollection)
      .aggregate(this.getTotalCountAggregationPipeline())
      .toArray()[0].totalDocs;
