class DataAssociation {
  ...
  insertSummaryDocuments(skipValue, batchEndValue) {
    try {
      let summaryResponse = this.clientDb
  ...
  insertDetailDocuments(skipValue, batchEndValue) {
    try {
      ...
      print("INSERTED: " + Object.values(detailResponse.insertedIds).length);
    ...
    if (clientSummaryObject) {
      ...
      if (!userAssociated) {
        ...
        let userAssociationDoc = {
          user: {
            ...dataAssociationDetailDoc.user,
            agency: dataAssociationDetailDoc.agency,
          },
    ...
    for (
      ...
      sourceDocumentsList.forEach((sourceDocument) => {
        ...
        if (createrUser && modifierUser && clientObj) {
          ...
          const allInvitees = Object.values(
            sourceDocument?.appointmentInvitees
          ).flatMap((users) => users);
          allInvitees.forEach((affiliatedUser) => {
            const associatedUser = this.getUserWithId(affiliatedUser);
            if (associatedUser) {
              if (
...
const constructorParameters = {
  ...
  findQuery: {
    ...
    appointmentInvitees: { $exists: true },
  },
};
